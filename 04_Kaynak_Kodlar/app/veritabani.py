# SQLite kalıcılık katmanıdır.
# Analiz üst bilgileri, nesne bazlı ölçümler ve kullanıcı geri bildirimleri ilişkisel olarak saklanır.
# Foreign key ve transaction kullanımı kayıt bütünlüğünü korur.
...
# Her işlem için kısa ömürlü bağlantı açılır.
# row_factory sayesinde sonuç satırlarına sütun adıyla erişilir ve foreign key kuralları etkinleştirilir.
def baglanti_ac():
...
# Mevcut veritabanını silmeden yeni sütun ekleyen hafif şema migrasyon yardımcısıdır.
...
# Servis başlangıcında tabloları ve indeksleri idempotent biçimde oluşturur.
# CREATE TABLE IF NOT EXISTS mevcut kurulumların bozulmasını engeller.
def veritabani_olustur():
    with baglanti_ac() as b:
        ...
        # Nesne kayıtları analiz kaydına foreign key ile bağlıdır.
        # ON DELETE CASCADE sayesinde analiz silindiğinde bağlı vida/somun kayıtları otomatik temizlenir.
...
# Analiz ve nesne kayıtları aynı transaction içinde yazılır.
# Ara adımda hata oluşursa kısmi/veri bütünlüğü bozuk kayıt bırakılmaz.
...
# Tarih ve minimum nesne sayısı filtrelerini dinamik SQL koşullarına dönüştürür.
# Sorgu parametreleri placeholder ile bağlanarak SQL injection riski azaltılır.
...
# Tek analizin üst bilgilerini ve bağlı nesnelerini getirir;
# özet sayılar, toplam alanlar ve ortalama güven değeri uygulama katmanı için burada hesaplanır.
def analiz_detayi_getir(analiz_id):
    ...
    for n in vidalar + somunlar:
        n.pop("tur", None)
        # Kalibrasyon katsayısı varsa piksel alanları mm²'ye dönüştürülür.
        # Katsayı yoksa yalnız ham px² ölçümleri korunur.
        if a["mm2_per_px2"]:
...
# Silme işlemi önce ilişkili dosya adlarını okur, sonra veritabanı kaydını kaldırır.
# Fiziksel dosyalar API katmanında bu dönen adlarla temizlenir.
...
# Yetim geri bildirim oluşmaması için hedef analiz kaydının varlığı ekleme öncesinde doğrulanır.
