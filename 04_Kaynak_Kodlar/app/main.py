# PartSense FastAPI uygulamasının giriş noktasıdır.
# Dosya yükleme, kalite kontrolü, model çıkarımı, veritabanı kaydı,
# geçmiş sorguları ve PDF/CSV raporlama uç noktalarını tek servis altında toplar.
...
# Uygulama logları tek bir adlandırılmış logger üzerinden üretilir; üretimde handler ve seviye merkezi olarak ayarlanabilir.
logger = logging.getLogger("gitek_vision")

# Çalışma dizinine bağımlılığı kaldırmak için upload, sonuç ve web dosyası yolları kaynak dosyaya göre hesaplanır.
...
# Veritabanı şeması servis başlarken bir kez doğrulanır ve eksik tablolar/sütunlar oluşturulur.
veritabani_olustur()
...
# CORS ayarı yerel ağdaki Flutter ve web istemcilerinin API'ye erişmesini sağlar. Üretimde izin verilen origin listesi daraltılmalıdır.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# Geri bildirim gövdesi Pydantic ile doğrulanır; tip dışı veya eksik alanlar API seviyesinde reddedilir.
...
# Sağlık uç noktası model, cihaz ve servis durumunu istemciye verir; Flutter bağlantı göstergesi bu endpoint'i periyodik sorgular.
@app.get("/saglik")
def saglik():
    """
...
# Bu endpoint model çalıştırmadan yalnız görüntü kalite uygunluğunu değerlendirir.
...
# Geçmiş analizleri tarih ve minimum nesne sayısı filtreleriyle listeler.
@app.get("/analizler")
...
# Analiz silindiğinde önce veritabanı kaydı, ardından ilişkili yükleme ve sonuç görselleri temizlenir.
...
# Analiz özetini ve nesne bazlı metrikleri Excel uyumlu UTF-8 BOM içeren CSV çıktısına dönüştürür.
def _csv_uret(d):
    out = io.StringIO()
    w = csv.writer(out)
...
# Analiz sonuçlarını arşivlenebilir ve paylaşılabilir PDF raporuna dönüştürür.
...
# Ana üretim endpoint'i: dosyayı doğrular, kalite kontrolü yapar, YOLO çıkarımını çalıştırır ve sonucu veritabanına kaydeder.
@app.post("/tahmin")
async def tahmin(
